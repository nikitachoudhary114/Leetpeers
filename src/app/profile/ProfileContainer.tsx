...
function AvatarSelectionSection({
  ...
  const currentAvatarData = CHARACTER_AVATARS.find(
    (a) => a.id === currentAvatar
  );
    ...
    <div className="bg-[var(--color-bg-secondary)] rounded-2xl border border-[var(--color-border)] p-6">
      ...
      {isEditing ? (
        ...
        <div className="flex items-center gap-4 mt-2">
          <Avatar
            src={currentAvatar}
            name={currentAvatarData?.name || "Select Avatar"}
            size="xl"
          />
          <div>
            ...
            {currentAvatarData && (
              <p className="text-sm text-[var(--color-text-muted)]">
                Click &quot;Change&quot; to select a different avatar
              </p>
            )}
          </div>
        </div>
